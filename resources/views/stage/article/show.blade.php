@extends('stage.layouts.app')

{{-- @section('title', $article->title) --}}

@section('content')
    <jumbotron v-cloak>
        <h4>{{ $article->title }}</h4>

        <h6>{{ $article->description }}</h6>

        <div class="header">
            <i class="icon-star"></i>{{ $article->user->name or 'null' }}，
            {{--@if(count($article->tags))--}}
                {{--<i class="ion-pricetag"></i>--}}
                {{--@foreach($article->tags as $tag)--}}
                    {{--<a href="{{ url('tag', ['tag' => $tag->tag]) }}">{{ $tag->tag }}</a>，--}}
                {{--@endforeach--}}
            {{--@endif--}}
            <i class="icon-calendar"></i>{{ \Carbon\Carbon::parse($article->published_at)->diffForHumans() }}
        </div>
    </jumbotron>

    <div class="article container">
        <div class="row">
            <div class="col-md-8 col-md-offset-2">

                {{$article->content}}
                {{--<parse :content="{{ $article->content }}"></parse>--}}

                {{--@if($article->is_original)--}}
                    <div class="publishing alert alert-dismissible alert-info">
                        <button type="button" class="close" data-dismiss="alert">×</button>
                        <p>由 xizy Yoo! 创作，采用 知识共享 署名-非商业性使用 4.0 国际 许可协议进行许可。</p>
                        <p>基于http://xless.cn上的作品创作。</p>
                    </div>
                {{--@endif--}}
                {{--@if(config('blog.social_share.article_share'))--}}
                    {{--<div class="footing">--}}
                        {{--<div class="social-share"--}}
                             {{--data-title="{{ $article->title }}"--}}
                             {{--data-description="{{ $article->title }}"--}}
                             {{--{{ config('blog.social_share.sites') ? "data-sites=" . config('blog.social_share.sites') : '' }}--}}
                             {{--{{ config('blog.social_share.mobile_sites') ? "data-mobile-sites=" . config('blog.social_share.mobile_sites') : '' }}--}}
                             {{--initialized></div>--}}
                    {{--</div>--}}
                {{--@endif--}}
            {{--</div>--}}
        </div>
    </div>

    @if(Auth::guest())
        <comment title="评论"
                 commentable-type="articles"
                 commentable-id="{{ $article->id }}"></comment>
    @else
        <comment title="评论"
                 username="{{ Auth::user()->name }}"
                 user-avatar="{{ asset('icon.JPG') }}"
                 commentable-type="articles"
                 commentable-id="{{ $article->id }}"
                 can-comment></comment>
    @endif

@endsection

@section('scripts')
    <script>
        hljs.initHighlightingOnLoad();
    </script>
@endsection
